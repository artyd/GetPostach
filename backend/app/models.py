...
class Contact(Base):
    ...
    email: Mapped[str | None] = mapped_column(String(400), index=True)
...
class Quote(Base):
    ...
    cas: Mapped[str | None] = mapped_column(String(120))
    spec: Mapped[str | None] = mapped_column(Text)
    price: Mapped[float | None] = mapped_column(Float)
    currency: Mapped[str | None] = mapped_column(String(64))
    unit: Mapped[str | None] = mapped_column(Text)
    # Free-text fields extracted from emails — no length cap (Postgres enforces VARCHAR len).
...
class ThreadState(Base):
    ...
    last_from: Mapped[str | None] = mapped_column(Text)
